...
int isu_mmu_page_fetch(isu_mmu_t mem, int p, unsigned long long *t){
	...
	case 0: /// fifo page replacement
		...
		*t += DISK_DELAY;
		...
		break;
	case 1:
		//int isu_mmu_page_rep_lru(mem, 
		break;
	case 2:	/// Clock hand replacement
		// page to remove mmu->hand
...
int isu_mmu_page_rep_lru(isu_mmu_t mem, isu_mem_req_t req, unsigned long long *t){
	...
	if(0==hit){
		isu_mem_req_set_access_hit(req, 1);
		//Set the time that it was referenced
		for(i=0;i<L1_SIZE;i++){
			if(mem->L1[i]->page==page){
				mem->L1[i]->access_time = *t;
			}
		...
		new=page;
//This is where LRU differs from FIFO
		old = mem->L1[0]->page;
		lru = mem->L1[0]->access_time;
		for(i=0;i<L1_SIZE; i++){
			if(mem->L1[i]->access_time < lru){
				old = mem->L1[i]->page;
				lru = mem->L1[i]->access_time;
			}
		}
		ret = isu_mmu_page_swap(mem, old, new, hit, t);
...
int isu_mmu_page_rep_clock(isu_mmu_t mem, isu_mem_req_t req, unsigned long long *t){
	...
	else if(0 < hit)
	{
		...
		/// now we figure out which one is to be replaced
		/// this is the one the hand is pointing to, provided the ref bit is not set to 1
