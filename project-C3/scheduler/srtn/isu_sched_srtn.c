...
isu_task_t srtn_get_next_task(void* _this, int remove)
{
	...
	{//if our current task still has time left
		if(this->running_task)
			if(remove==0){
				isu_task_t i;
				int flag=0;
				for_each(i, this->task_list){
					if((this->running_task->remaining_time)<(i->remaining_time)){
						isu_llist_insert_before(this->task_list, this->running_task);
						flag=1;
						break;
					}
				}
				if(flag==0)
					isu_llist_push(this->task_list, this->running_task, ISU_LLIST_TAIL);
			}
		this->running_task = isu_llist_pop(this->task_list, ISU_LLIST_HEAD);
//Just need to run the first thing in the list because we sort it on creation and addition
            	if(this->running_task)
			isu_print(PRINT_DEBUG, "Run task with the least amount of time left");
        	else
