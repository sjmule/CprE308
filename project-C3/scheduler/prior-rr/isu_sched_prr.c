...
isu_task_t prr_get_next_task(void* _this, int remove)
{
	...
	else
	{
		if(this->running_task)
			if(this->running_task->remaining_time>0)
			{
				isu_llist_push(this->task_list[this->running_task->priority], this->running_task, ISU_LLIST_TAIL);
			}
		int i=0;
		for(i; i<4; i++)
		{
			this->running_task = isu_llist_pop(this->task_list[i], ISU_LLIST_HEAD);
			if(this->running_task)
			{
				isu_print(PRINT_DEBUG, "Run task with priority %d", this->running_task->priority);
				break;
			}
		}
