...
{
//#warning queue_push not finished //this is probably finished
	...
	else
	{
		...
		{
			case QUEUE_HEAD:
				/// @todo push this node to the head and move the head //ok
				node->next = list->head;
				list->head->prev = node;
				list->head = node;
				...
				break;
			case QUEUE_TAIL:
				/// @todo push this node to the tail and move the tail //ok
				node->prev = list->tail;
				list->tail->next = node;
				list->tail = node;
	...
	/// @todo increment the list count  //ok
	sem_post(&list->count);
	
	/// @todo unlock the list  //ok
...
{
//#warning queue_pop not finished //this is probably finished
	// assert the list is valid
	assert(list > 0);
	queue_node_t* node;
	void* data;
	
	/// @todo wait for the list to contain something //ok
	sem_wait(&list->count);
