...
int main(int argc, char* argv[])
{
	...
	if(arguments.verbose_mode == 2) printf("Creating socket\n");

	ServAddr.sin_family = AF_UNIX;
	ServAddr.sin_port = 54837;
	...
	if(arguments.verbose_mode == 2) printf("Waiting for clients\n");

	s = accept(fd, (struct sockaddr*)&ClientAddr, &ClientAddrLen);

	if(arguments.verbose_mode == 2) printf("Waiting for jobs\n");
