...
{
//#warning print_job_tostring not finished //Probably finished
	///@todo  //ok
	char *str;
	// the student should fill in this to print more useful information
	if(asprintf(&str, "Print Job #%d\nPrinting file: %s on driver: %s\n", job->number, job->description, job->driver_name))
		return NULL;
	return str;
}
